...
const { king } = require(__dirname + "/../felix/king");
const { format } = require(__dirname + "/../felix/mesfonctions");
const os = require('os');
...
const fetchGitHubStats = async () => {
    try {
        const response = await axios.get("https://github.com/devhanstz/HANS-MD-V2");
...
king({ nomCom: "menu", aliases: ["liste", "helplist", "commandlist"], categorie: "SYSTEM" }, async (message, client, config) => {
    const { ms, respond, prefix, nomAuteurMessage } = config;
    const commands = require(__dirname + "/../felix/king").cm;
